...
public class Crear extends Thread{
	...
	private static String DB_PASSWORD="skoa";
	private static String DB_NAME="Skoa_Bd";
	 ...
	 */
	 ...
	 private String comandoLinux="/usr/bin/mysql -u "+DB_USER+" -p"+DB_PASSWORD+" "+DB_NAME;
	...
	public void run() {
        ...
        try {
            ...
            con= DriverManager.getConnection("jdbc:mysql://localhost/mysql",DB_USER,DB_PASSWORD);
            Statement st = con.createStatement();
            
            //SE CREA LA BASE DE DATOS SINO EXISTE
            String accion="create database if not exists "+DB_NAME;
            st.execute(accion);
            st.close();
            con.close();
            ...
            if (os.indexOf("Win")>=0) {
                ...
                comandoWindows=comandoWindows+"/bin/mysql -u "+DB_USER+" -p"+DB_PASSWORD+" "+DB_NAME;
